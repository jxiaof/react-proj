  ...
  return (
    <PageTransition>
      <MainLayout hideBottomNav={false} fullWidth>
        {/* 
          使用 Flex 布局而非计算高度
          - 外层：flex flex-col，height 由 MainLayout 容器决定
          - 侧边栏：fixed 定位（不占空间）
          - 主区域：flex-1，自动填充剩余空间
          - 消息区：flex-1 overflow-y-auto，仅此处滚动
          - 输入框：flex-shrink-0，始终显示，不滚动
        */}
        <div className="flex h-full overflow-hidden bg-background">
          ...
          {/* Main Chat Area - Flex 三层布局 */}
          <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
            {/* Header - 固定高度，不参与 flex 分配 */}
            ...
            {/* Messages Container - 唯一滚动区域，flex-1 占满剩余空间 */}
            <div 
            ...
            {/* Input Area - 固定底部，不参与 flex 分配 */}
            {conversationId && (
              <div className="flex-shrink-0 border-t-2 border-primary/20 bg-background/95 backdrop-blur-sm px-3 md:px-4 py-3 md:py-4 safe-area-pb">
                <ChatInput
                  ref={chatInputRef}
                  value={inputValue}
